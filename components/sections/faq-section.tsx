  ...
  return (
    <Section id="faq">
      ...
      <Reveal delay={120} rootMargin="0px 0px 15% 0px" threshold={0}>
        <div className="mt-16 overflow-hidden rounded-3xl border border-neutral-900/10 bg-white/90 shadow-sm shadow-neutral-900/5">
          <Accordion type="single" collapsible className="divide-y divide-neutral-900/10">
            {faqs.map((faq, index) => (
              <AccordionItem key={faq.question} value={`faq-${index + 1}`} className="border-b-0">
                <AccordionTrigger className="px-6 py-6 text-left hover:bg-cyan-50/60 hover:no-underline focus-visible:ring-cyan-700/30 md:px-8 [&>svg]:text-neutral-500">
                  <span className="font-mono text-sm uppercase tracking-[0.18em] text-neutral-950 md:text-base">
                    {faq.question}
                  </span>
                </AccordionTrigger>
